...
(function () {
  ...
  /* ------------------------------------------------------------------
     SUPABASE — donde queda guardada cada solicitud.

     Se usa el proyecto de PresAdos. Las dos constantes salen del panel, en
     Project Settings → API. Si se vacían, esto deja de hacer nada y el
     formulario sigue funcionando por WhatsApp.

     La clave «publishable» es PÚBLICA por diseño — el propio panel lo dice —
     y vive en el navegador de todo el que abra la página. Lo que protege la
     tabla es RLS: `solicitudes` tiene una única política, de INSERT. Probado
     el 13 de agosto de 2026: con esta clave se escribe una solicitud, pero
     leerla devuelve vacío y borrarla no alcanza ninguna fila.

     NUNCA pegar aquí una clave `sb_secret_…` ni `service_role`: se saltan el
     RLS entero y este repositorio es público.
     ------------------------------------------------------------------ */
  var SUPABASE_URL = 'https://zkfnlosaecsrdtxhoymm.supabase.co';
  var SUPABASE_KEY = 'sb_publishable_85nvuxPFz6ZrG01V2RI6lw_ph0QV-79';
  ...
  function recoger(form) {
    ...
    for (var i = 0; i < campos.length; i++) {
      var c = campos[i];
      if (c.name === 'sitio') continue;   /* campo trampa, se mira en atender() */
  ...
  /* Guarda la solicitud en Supabase.
     ...
     No se espera la respuesta a propósito. WhatsApp tiene que abrirse en el
     mismo tick del submit o el navegador lo toma por ventana emergente y lo
     bloquea; si aquí hubiera un await, se bloquearía. `keepalive` deja que la
     petición llegue a término aunque el navegador ya haya cambiado de pestaña.

     Si falla, se traga el error: el visitante ya va camino de WhatsApp y no
     gana nada viendo un fallo técnico. */
  function guardar(datos) {
    if (!SUPABASE_URL || !SUPABASE_KEY) return;
    fetch(SUPABASE_URL + '/rest/v1/solicitudes', {
      method: 'POST',
      headers: {
        'apikey': SUPABASE_KEY,
      ...
      body: JSON.stringify({
        ...
        licencias: datos.licencias || null,
  ...
  function atender(form) {
    form.addEventListener('submit', function (ev) {
      ...
      /* Campo trampa. Es invisible, así que una persona nunca lo llena; si
         viene con algo es un robot. No se guarda nada, pero se responde como
         si todo hubiera ido bien para que no se dé cuenta y vuelva a probar. */
      var trampa = form.querySelector('[name="sitio"]');
      if (trampa && trampa.value) {
        avisar(form, 'Recibido. Le respondemos en breve.');
        return;
      }
      ...
      /* Queda guardada pase lo que pase después. Este es el registro que no
         se pierde aunque el visitante no llegue a pulsar enviar en WhatsApp. */
      guardar(datos);
